...
#### sample function
def get_alpha_cum(t):
    return torch.where(t >= 0, torch.cos((t + 0.008) / 1.008 * math.pi / 2)**2, 1.0).clamp(1e-4, 1-1e-4).cuda()

def get_z_t(x_0, t, eps=None):
    alpha_cum = get_alpha_cum(t)[:, None, None, None, None]
    eps = torch.randn_like(x_0).cuda() if eps is None else eps
...
def sto_ddim_sample(x_0, z_tp1, t, t_p1, eta=0.2):
    """
    Stochastic DDIM sampling step.
    
    Args:
        x_0: predicted clean sample from model
        z_tp1: sample from previous step
        t, t_p1: current and previous timestep scalars (normalized or indices)
        eta: controls stochasticity; 0 = deterministic DDIM, 1 = DDPM-level noise
    """
    # Compute predicted noise
    eps = get_eps_x_t(x_0, z_tp1, t_p1)
    ...
    sigma_t = eta * torch.sqrt((1 - alpha_cum_tp1 / alpha_cum_t))
    # sigma_t = eta * torch.sqrt((1 - alpha_cum_t / alpha_cum_tp1) * (1 - alpha_cum_tp1) / (1 - alpha_cum_t))
    # print(sigma_t)
    # Add random noise
    ...
    x_t = (
        torch.sqrt(alpha_cum_t) * x_0 +
        torch.sqrt(1 - alpha_cum_t - sigma_t**2) * eps +
        sigma_t * noise
    )
